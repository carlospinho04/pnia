...
import com.carlos.pnia.domain.BusinessSectorError
import io.circe.syntax._
import org.http4s.circe.{CirceEntityDecoder, CirceEntityEncoder}
import org.http4s.dsl.Http4sDsl
import org.http4s.{HttpRoutes, Request, Response}

class HttpErrorHandler[F[_] : Sync, E <: Exception] extends Http4sDsl[F] with CirceEntityEncoder with CirceEntityDecoder {
  private val handler: domain.Error => F[Response[F]] = {
    case BusinessSectorError(error) => BadRequest(error.asJson)
  }
